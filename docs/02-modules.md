...
| `documents` | Договор, счёт, акт: шаблоны, PDF, нумерация, **подписание УКЭП** | `documents` | deal | 24 + 80 |
| `payments` | Вебхуки, **три схемы удержания**, **движок ставок**, выплаты | `payments` | deal | 16 + 88 |
...
> **Оценка пересчитана 27.08.2026 после решений по деньгам.**
>
...
> | Саморегистрация подрядчика и очередь модерации | 16 |
> | Проверка ИНН по открытым данным налоговой (решение Q11) | 12 |
> | Рекомендации: правила плюс метод `ai-service` | 12 |
> | Второй вход в сделку: снимок цены, переходы, тесты обоих путей | 10 |
> | **Три схемы удержания вместо одной** (`docs/09-money-holding.md`) | +32 |
> | **Движок ставок: две лестницы скидок** (`docs/10-commission-rates.md`) | +56 |
> | **Подписание документов УКЭП** (`docs/11-electronic-signature.md`) | +80 |
> | **Итого к базовым 252** | **+276** |
>
> Новая база 528 ч, с коэффициентом переделок 1,2 — **около 634 часов**.
> При 62 ч/мес это 10,2 месяца чистой работы и **13–15 месяцев календаря**
> против семи по исходному плану.
>
> Решение «делаем всё сразу» принято 27.08.2026, до того как выяснилась цена
> квалифицированной подписи. Она добавилась последней и одна стоит около
> трёх месяцев календаря — стоит перечитать это решение, зная её.
> Ответ на Q17 добавит к этому ещё 20 часов, если путей подписания будет два.
> Молча я не сокращаю: сокращения перечислены в `docs/06-open-questions.md`.
