...
| `payments` | Вебхуки, **три схемы удержания**, **движок ставок**, выплаты | `payments` | deal | 16 + 80 |
...
> | **Движок ставок комиссии** (`docs/10-commission-rates.md`) | +48 |
> | **Итого к базовым 252** | **+188** |
>
> Новая база 440 ч, с коэффициентом переделок 1,2 — **около 528 часов**.
> При 62 ч/мес это 8,5 месяца чистой работы и 10–12 месяцев календаря
> против семи по исходному плану.
>
> Что можно вернуть, не теряя ничего, а только сместив по времени:
> схемы Б и В делать после пилота (−22 ч, общий контур пишется сразу),
> акционные и индивидуальные ставки — после пилота (−30 ч, структура правил
> пишется сразу). Вместе это 52 часа и примерно месяц календаря.
> Цена названа один раз; решение за разработчиком, молча не сокращаю.
