...
**Запуск на своей машине под Windows — отдельно: `docs/13-local-server.md`
и папка `ospanel/` рядом.** Здесь — боевой сервер.
...
То же и с `ospanel/*.bat`: логику первой настройки можно было проверить целиком
(она вынесена в `scripts/local-setup.mjs` и запускается на любой системе),
а сами bat-файлы — нет, машины с Windows у агента нет. Они намеренно короткие
и состоят из вызовов `pnpm`, чтобы ломаться было негде.
